...
sequence:	.byte 20 #store a sequence of 20 ints
	.text

main:
	#generate our pseudorandom sequence
	jal generateSequence
	...
	#encode our string according to the sequence
	la $t0, strToEncode
	la $s6, sequence
	jal encode
	...
	#find its most common character
	la $t0, charCounter2
	li $t2, 0
	li $t3, 0
	li $t4, 0
	...
	#exactly as before, but going from encoded to decoded.
	la $t3, strToDecode
	jal promptFunct
...
decode:
	...
	sub $t1, $t1, $s7 #subtract the ith sequence int. 
shifted2:	
...
encode:	#iterate over the string. If we reach a line feed or
	...
	add $t1, $t1, $s7 #shift by ith sequence value
shifted:	
	bgt $t1, 'z', wrapBack
...
checkKey: #adjust to fall within the range ([-26,26])
	bgt $s0, 26, bigKey
	blt $s0, -26, smallKey
	jr $ra
...
generateSequence:
	li $t0, 0 #counter
	la $t1, sequence
	li $a1, 256 #max value non inclusive
	li $v0, 42 
fill:	
	syscall #a0 contains random int 0-255

	sb $a0, ($t1) #store in our sequence
	addi $t0, $t0, 1
	addi $t1, $t1, 1
	blt $t0, 20, fill
	jr $ra
